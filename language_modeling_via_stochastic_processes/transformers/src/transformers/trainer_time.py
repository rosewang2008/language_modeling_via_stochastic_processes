...
"""
The Trainer_Time class, to easily train a 🤗 Transformers from scratch or finetune it on a new task.
"""
...
class Trainer_Time:
    """
    Trainer_Time is a simple but feature-complete training and eval loop for PyTorch, optimized for 🤗 Transformers.
    ...
    Args:
        model (:class:`~transformers.PreTrainedModel` or :obj:`torch.nn.Module`, `optional`):
            ...
            .. note::
                ...
                :class:`~transformers.Trainer_Time` is optimized to work with the :class:`~transformers.PreTrainedModel`
        ...
        train_dataset (:obj:`torch.utils.data.dataset.Dataset` or :obj:`torch.utils.data.dataset.IterableDataset`, `optional`):
            ...
            processes (and the Trainer_Time will manually set the seed of this :obj:`generator` at each epoch) or have a
        ...
        model_init (:obj:`Callable[[], PreTrainedModel]`, `optional`):
            ...
            :meth:`~transformers.Trainer_Time.train` will start from a new instance of the model as given by this function.
        ...
        callbacks (List of :obj:`~transformers.Trainer_TimeCallback`, `optional`):
            A list of callbacks to customize the training loop. Will add those to the list of default callbacks
            detailed in :doc:`here <callback>`.

            If you want to remove one of the default callbacks used, use the :meth:`Trainer_Time.remove_callback` method.
    ...
    def __init__(
        ...
        if model is None:
            if model_init is not None:
                ...
                raise RuntimeError("`Trainer_Time` requires either a `model` or `model_init` argument")
        else:
            if model_init is not None:
                warnings.warn(
                    "`Trainer_Time` requires either a `model` or `model_init` argument, but not both. "
        ...
        if model_init is not None and (self.optimizer is not None or self.lr_scheduler is not None):
            raise RuntimeError(
                ...
                "You should subclass `Trainer_Time` and override the `create_optimizer_and_scheduler` method."
            )
    ...
    def add_callback(self, callback):
        """
        Add a callback to the current list of :class:`~transformer.Trainer_TimeCallback`.

        Args:
           callback (:obj:`type` or :class:`~transformer.Trainer_TimeCallback`):
               A :class:`~transformer.Trainer_TimeCallback` class or an instance of a :class:`~transformer.TrainerCallback`.
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.train_dataset is None:
            raise ValueError("Trainer_Time: training requires a train_dataset.")
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if eval_dataset is None and self.eval_dataset is None:
            raise ValueError("Trainer_Time: evaluation requires an eval_dataset.")
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        Trainer_Time's init through :obj:`optimizers`, or subclass and override this method (or :obj:`create_optimizer`
    ...
    def create_optimizer(self):
        ...
        We provide a reasonable default that works well. If you want to use something else, you can pass a tuple in the
        Trainer_Time's init through :obj:`optimizers`, or subclass and override this method in a subclass.
        """
        ...
        **kwargs,
        ...
        Args:
            resume_from_checkpoint (:obj:`str` or :obj:`bool`, `optional`):
                ...
                :class:`~transformers.Trainer_Time`. If a :obj:`bool` and equals `True`, load the last checkpoint in
                `args.output_dir` as saved by a previous instance of :class:`~transformers.Trainer_Time`. If present,
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        for k, v in inputs.items():
            if isinstance(v, torch.Tensor):
                ...
                #     # Reshaping to num_layers in transformer
                #     # List of tf.Tensor of length config.n_layers,
                ...
                #     emb_size = int(self.model.config.n_embd / num_heads)
                #     past_key_values = []
                #     for _ in range(num_layers):
                #         elem = inputs[k].expand(2, batch_size, num_heads, seq_len, emb_size)
                #         past_key_values.append(elem)

                #     inputs[k] = past_key_values
    ...
    def _save_tpu(self, output_dir: Optional[str] = None):
        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                ...
                logger.info("Trainer_Time.model is not a `PreTrainedModel`, only saving its state dict.")
                state_dict = self.model.state_dict()
    ...
    def evaluation_loop(
        ...
        """
        Prediction/evaluation loop, shared by :obj:`Trainer_Time.evaluate()` and :obj:`Trainer.predict()`.
    ...
    def push_to_hub(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            kwargs:
                Additional keyword arguments passed along to :meth:`~transformers.Trainer_Time.create_model_card`.
