...
class GPT2TimeModel(GPT2PreTrainedModel):
    ...
    def _get_cl_embeddings(self, raw_text, cl_feats, seq_cl_feats, input_ids, seq_len):
        # NOTE splitting text as done https://github.com/rosewang2008/nonstationarity/blob/main/code/src/datasets/wikisection.py#L175
        ...
        if seq_cl_feats is not None and cl_feats is not None: # Used in evaluation
            ...
            if input_ids.shape[0] > 1:
                ...
                cl_feats = torch.clone(self._last_beam_cl_feats)
                # TODO need to update this
                raise ValueError()
                for seq_idx, beam_seq in enumerate(input_ids):
                    if beam_seq[-1] == 764: # eos
                        self._transition_cl[seq_idx] = True
                    elif self._transition_cl[seq_idx]: # last id was eos
                        if self._cur_cl_idx[seq_idx] + 1 < seq_cl_feats.shape[0]:
                            self._cur_cl_idx[seq_idx] += 1
                            cl_feats[seq_idx] = seq_cl_feats[self._cur_cl_idx[seq_idx]]
                self._last_beam_cl_feats = torch.clone(cl_feats)
                ...
                # if input_ids[0][0] == 764: # end of current sentence
                #     # self._transition_cl = True
                #     if self._cur_cl_idx + 1 < seq_cl_feats.shape[0]: # next idx still within seq
                #         self._cur_cl_idx += 1
                # elif self._transition_cl: # last id was end of a sentence
                #     self._transition_cl = False
                #     if self._cur_cl_idx + 1 < seq_cl_feats.shape[0]: # next idx still within seq
                #         self._cur_cl_idx += 1
                #         cl_feats = seq_cl_feats[self._cur_cl_idx]

        # elif cl_feats is None:
        #     # Running some checks
        #     assert len(raw_text) == 1
        #     assert self._config.embedding_type, "must set embedding type"
        #     if "wikisection" in self._config.dataset_name:
        #         split_pattern = ". "
        #     else: # stories
        #         split_pattern = " . "
        #     split_text = raw_text[0].split(split_pattern)[:-1]

        #     if not split_text[0]:
        #         split_text = split_text[1:]
        #     # NOTE: new way of getting embeddings:
        #     # get embedding of first section sentence.

        #     if self._config.embedding_type == "firstSentence":
        #         raise ValueError("old code, dont use this setting")
        #         cl_input_ids, cl_attention_mask = self.cl_tokenize_text([split_text[0]])
        #         cl_feats = self.CL_MODEL.forward(input_ids=cl_input_ids, attention_mask=cl_attention_mask) # 1, feat_size
        #     elif self._config.embedding_type == "entireSection":
        #         # Off-by-one index
        #         end_idxs = [i-1 for i, x in enumerate(input_ids[0]) if x == 764 and i != 0] # end of sentence
        #         end_idxs += [input_ids.shape[-1]] # end of document
        #         while len(end_idxs) != len(split_text):
        #             diffs = [end_idxs[i+1] - end_idxs[i] for i in range(len(end_idxs[:-2]))] # dont include last element in removal
        #             min_idx = np.argmin(diffs)
        #             end_idxs.remove(end_idxs[min_idx+1])

        #         assert len(end_idxs) == len(split_text)
        #         # Collect feats for each sentence.
        #         cl_feats = []
        #         for split in split_text:
        #             if "wikisection" not in self._config.dataset_name:
        #                 split += split_pattern
        #             cl_input_ids, cl_attention_mask = self.cl_tokenize_text([split])
        #             cl_feat = self.CL_MODEL.forward(input_ids=cl_input_ids, attention_mask=cl_attention_mask) # 1, feat_size
        #             cl_feats.append(cl_feat)

        #         cl_seq = []
        #         # Expand feats by sentence token length
        #         prev_sentence_eos_idx = 0
        #         for sentence_num, cl_feat in enumerate(cl_feats): # last eos should be a start
        #             sentence_eos_idx = end_idxs[sentence_num]
        #             # print(sentence_eos_idx - prev_sentence_eos_idx)
        #             cl_seq += [cl_feat for _ in range(sentence_eos_idx - prev_sentence_eos_idx)]
        #             prev_sentence_eos_idx = sentence_eos_idx

        #         cl_feats = torch.cat(cl_seq)
        #         try:
        #             assert cl_feats.shape[0] == input_ids.shape[-1]
        #         except:
        #             import pdb; pdb.set_trace() # TODO assert length same
        #         generated_by_raw_text = True

        #     else:
        #         raise ValueError()
        #         # NOTE: old way of getting embeddings; approximate association of embeddings with token length
        #         if cl_feats is None:
        #             generated_by_raw_text =True
        #             split_pattern = ". "
        #             split_raw_text = [_ + split_pattern for _ in raw_text[0].split(split_pattern)]
        #             split_raw_text[-1] = split_raw_text[-1].rstrip(split_pattern)
        #             cl_feat_list = []
        #             cl_result = []
        #             cl_lens = []
        #             # First collect lengths of sentences
        #             for split_text in split_raw_text:
        #                 cl_input_ids, cl_attention_mask = self.cl_tokenize_text([split_text])
        #                 cl_feats = self.CL_MODEL.forward(input_ids=cl_input_ids, attention_mask=cl_attention_mask)
        #                 cl_feat_list.append(cl_feats)
        #                 cl_lens.append(cl_input_ids.shape[-1])

        #             # Distribute embeddings by ratio
        #             total_cl_len = sum(cl_lens)
        #             for i, (cl_len, cl_feat) in enumerate(zip(cl_lens, cl_feat_list)):
        #                 ratio_len = int(cl_len/total_cl_len * input_ids.shape[-1])
        #                 if i == len(split_raw_text) - 1: # last element
        #                     cl_result += [cl_feat for _ in range(input_ids.shape[-1] - len(cl_result))]
        #                 else:
        #                     cl_result += [cl_feat for _ in range(ratio_len)]

        #             cl_feats = torch.cat(cl_result)

        if cl_feats is None and seq_cl_feats is not None:
            cl_feats = seq_cl_feats

        if self._config.use_noisy_embeddings:
            cl_feats += torch.normal(0, 1.0, size=cl_feats.shape).to(self.device)
    ...
    def _get_section_ids(self, input_ids, section_ids, seq_section_ids):
        ...
        if seq_section_ids is not None: # Used in evaluation
            ...
            if input_ids.shape[0] > 1:
                ...
                # if input_ids[0][0] == 764: # end of current sentence
                #     self._transition_section = True
                # elif self._transition_section: # last id was end of a sentence
                #     self._transition_section = False
                #     if self._cur_section_idx + 1 < seq_section_ids.shape[0]: # next idx still within seq
                #         self._cur_section_idx += 1
                #         section_ids = seq_section_ids[self._cur_section_idx]
    ...
    def forward(
        ...
        if self._print_debug:
            # print("[DEBUG_PPL] RAW TEXT {}".format(raw_text))
            # print("[DEBUG_PPL] INPUT IDS {}".format(input_ids[0, :20])) # TODO should check that the ids are the same across
        ...
        if self._config.debug_ids:
            raise ValueError("not used anymore")
            # GOAL: debug why section ids are not useful in LM...
            # EXPERIMENT: word level embeddings
            if self._config.use_section_ids or self._config.use_section_ids:
                assert ValueError('dont use this at same time')
            # encode next word
            next_input_embeds = self.wte(input_ids)
            next_input_embeds = next_input_embeds[:, 1:]
            next_input_embeds = torch.cat((next_input_embeds, next_input_embeds[:, -1:]), dim=1)
            hidden_states += next_input_embeds
