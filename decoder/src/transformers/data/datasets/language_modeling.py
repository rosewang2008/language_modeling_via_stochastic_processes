...
class Wikisection_BySectionDataset(TextDataset):
    """
    ...
    def __init__(self,
        ...
        super(Wikisection_BySectionDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
        ...
        with open(self.file_path, encoding="utf-8") as f:
            for idx, row in enumerate(f.readlines()):
                section_name = list(filter(lambda x: x in row, SECTIONS))[0]
                section_id = self.section2id[section_name]
                if row.strip():
                    ...
                    self.lengths[section_name].append(len(tokenized_text))

                    if len(tokenized_text) >= block_size:
                        for i in range(0, len(tokenized_text) - block_size + 1, block_size):
                            # Truncate in block of block_size
                            example = tokenizer.build_inputs_with_special_tokens(tokenized_text[i : i + block_size])
                            cl_text = tokenizer.decode(example)
                            if i == 0:
                                # Remove section id
                                cl_text = self._clean2cltext(cl_text)
                            cl_text = cl_text.replace(".\n", ". ")
                            cl_text = cl_text.replace("<|endoftext|> ", "")
                            cl_text = cl_text.replace("<|endoftext|>", "")

                            self.examples.append(example)
                            self.raw_texts.append(tokenizer.decode(example))
        ...
        print("example: {}".format(self.examples[0]))
        print("section_id: {}".format(self.section_ids[0]))
        print("raw_text: {}".format(self.raw_texts[0]))
        print("cl_text: {}".format(self.cl_texts[0]))

        self.labels = copy.deepcopy(self.examples)

    def _clean2cltext(self, text):
        cl_text = text.replace(".\n", ". ")
        cl_text = cl_text[cl_text.index('] ')+2:]
        return cl_text
...
#### For classification
class WikisectionClassificationDataset(TextDataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self,
        ...
        super(WikisectionClassificationDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
            ...
            for idx, row in enumerate(f.readlines()):
                if row.strip():
                    ...
                    tokenized_text = tokenizer.convert_tokens_to_ids(
                        tokenizer.tokenize(row))
                    sec_id = 0
                    for text in split_row:
                        if not text: # empty string
                            continue
                        text += "."
                        # Whether to update section id label
                        for tok in self.section_names:
                            if tok in text:
                                sec_id = self.section_names.index(tok)
                                text = text.replace(tok+" ", "")
                        # Tokenization similar to preprocess_f in text-classificatio
                        tokenized = tokenizer(text,
                                              padding='max_length',
                                              max_length=self.max_seq_len,
                                              truncation=True)['input_ids']
                        self.examples.append(tokenized)
                        self.labels.append([sec_id])
    ...
    def __getitem__(self, i):
        return (torch.tensor(self.examples[i], dtype=torch.long),
                # torch.tensor(self.labels[i], dtype=torch.int32),
                torch.tensor(self.labels[i], dtype=torch.long),
                )
#### For clm
...
class WikisectionDataset(TextDataset):
    ...
    def set_cl_tokenizer(self):
        cl_encoder_name = self.cl_model.__class__.__name__
        if "GPT2" in cl_encoder_name:
            self.cl_tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
            self.cl_tokenizer.pad_token = self.cl_tokenizer.eos_token
        elif "BERT" in cl_encoder_name:
            self.cl_tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
        else:
            raise ValueError()
    ...
    def process_dataset(self):
            ...
            for idx, row in enumerate(f.readlines()):
                if row.strip():
                    ...
                    if len(tokenized_text) >= block_size:
                        n_big += 1
                        skip_block = 100
                        for i in range(0, len(tokenized_text) - block_size + 1, skip_block):
                            # Truncate in block of block_size
                            example = self.tokenizer.build_inputs_with_special_tokens(tokenized_text[i : i + block_size])
                            self.examples.append(example)
                            # split by section ids
                            section_ids, last_section_id = self._determine_section_ids(example, last_section_id)
                            self.section_ids.append(section_ids)
                            # CL text - raw
                            cl_text = self.tokenizer.decode(example)
                            if i == 0:
                                # Remove section id
                                cl_text = self._clean2cltext(cl_text)
                            cl_text = cl_text.replace(".\n", ". ")
        ...
        self.labels = copy.deepcopy(self.examples)
        print(f"big: {n_big} vs small: {n_small}")
        for k, v in self.lengths.items():
            print("[ {} ] {}+-{}".format(k, np.mean(v), np.std(v) ))
    ...
    def _full_section_ids(self, tokenized_text, last_section_id):
        ...
        if 'long' in self.file_path:
            # For long wikisection
            start_idx = 0
            ...
            for tok in section_tokens[1:]: # skip abstract - always first and present
                if tok in tokenized_text:
                    ...
                    # Track length
                    self.lengths[self.section_names[section_tokens.index(cur_tok)]].append(
                        end_idx + 1 - start_idx)
    ...
    def __getitem__(self, i):
        return (torch.tensor(self.examples[i], dtype=torch.long),
                ...
                # torch.tensor(self.cl_embeddings[i], dtype=torch.long),
                # self.raw_texts[i]
                self.cl_texts[i]
                )

class WikisectionFilterLargeDataset(WikisectionDataset):
    def __init__(self,
                 cl_model,
                 tokenizer: PreTrainedTokenizer,
                 file_path: str,
        ...
        # self.cl_offset = 10
        self.cl_offset = 0
        super(WikisectionFilterLargeDataset, self).__init__(
                 cl_model=cl_model,
                 tokenizer=tokenizer,
                 file_path=file_path,
...
class StoriesDataset(TextDataset):
    ...
    def __init__(self,
        ...
        super(StoriesDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path="/nlp/scr/rewang/nonstationarity_final/code/scratch/wikisection_withSections.train.txt", # file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
                 cache_dir=cache_dir,
        )
...
class StoriesClassificationDataset(TextDataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self,
        ...
        super(StoriesClassificationDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
        ...
        with open(self.file_path, encoding="utf-8") as f:
            for idx, row in enumerate(f.readlines()):
                if row.strip():
                    ...
                    # Text for GPT2
                    tokenized_text = self.tokenizer.convert_tokens_to_ids(
                        self.tokenizer.tokenize(cl_text))

                    if len(tokenized_text) >= self.block_size:
                        num_filtered+=1
                    else:
                        # Split by prompt
                        tokenized = self.tokenizer(cl_text,
                                              padding='max_length',
                                              max_length=self.max_seq_len,
                                              truncation=True)['input_ids']
                        response_idx = tokenized.index(28997)
                        self.wps.append(tokenized[:response_idx])
                        self.stories.append(tokenized[response_idx:])
                        self.labels.append([1])

    def __len__(self):
        return len(self.wps) * 2
    ...
    def __getitem__(self, i):
        label = i // len(self.wps)
        idx = i % len(self.wps)
        if label: # positive pair
            example = self.wps[idx] + self.stories[idx]
        else:
            random_idx = np.random.randint(0, len(self.wps))
            example = self.wps[random_idx] + self.stories[idx]
            example = example[:512]
...
class StoriesDataset_SplitSentence(TextDataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self,
        ...
        super(StoriesDataset_SplitSentence, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
        ...
        self.section_ids = []
        self.raw_texts = []
        raise ValueError("need to implement cl_texts for contrastive encoder.")
        with open(self.file_path, encoding="utf-8") as f:
            for idx, row in enumerate(f.readlines()):
                if row.strip():
                    row = row[len("[ WP ]"):]
                    idx = row.index('[ RESPONSE ]')
                    prompt = row[:idx]
                    story = row[idx+len("[ RESPONSE ]"):]
                    split_story = story.split(". ")
                    split_story = [prompt] + [text + ". " for text in split_story]

                    for sentence in split_story:
                        if sentence == " ":
                            continue
                        tokenized_text = tokenizer.convert_tokens_to_ids(
...
class RecipeDataset(TextDataset):
    ...
    def __init__(self,
                 ...
                 use_section_null: bool,
                 special_words:list,
                 data_dir="/nlp/scr/rewang/data/recipe_nlg/dataset",
                 overwrite_cache=False,
                 cache_dir: Optional[str] = None,
                 name: str = 'recipe'
                 ):
    ...
    def _set_indices(self):
        ...
        if self.train:
            ...
            self.start_idx, self.end_idx = 0, 4_000 # NOTE most update to date
            # self.start_idx, self.end_idx = 0, 100 # NOTE for debugging / testing
        else:
            # self.start_idx, self.end_idx = 500_000, 750_000
            self.start_idx, self.end_idx = 500_000, 501_000 # NOTE most up-to-date
            # self.start_idx, self.end_idx = 100, 500 # NOTE for debugging / testing
    ...
    def _process_dataset(self):
        ...
        for doc_id in tqdm(range(self.start_idx, self.end_idx)):
            ...
            if len(tokenized_text) >= self.block_size:
                ...
                self.cl_texts.append(cl_text)
                # self.get_cl_embeddings(gpt2_text=row)
...
class WikihowDataset(RecipeDataset):
    ...
    def _set_indices(self):
        # SEE REPO <NONSTATIONARITY> FOR INDEXES
        if self.train:
            # self.start_idx, self.end_idx = 0, 40_000
            ...
            # self.start_idx, self.end_idx = 40_000, 41_000
            # self.start_idx, self.end_idx = 100, 300
...
class RecipeClassificationDataset(StoriesClassificationDataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self,
        ...
        self.train = 'train' in self.file_path
        self._set_indices()
        super(RecipeClassificationDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
        ...
        self.label_list = [_ for _ in range(len(self.special_words))]

    def _set_indices(self):
        # SEE REPO <NONSTATIONARITY> FOR INDEXES
        if self.train:
            # self.start_idx, self.end_idx = 0, 500_000
            self.start_idx, self.end_idx = 0, 20_000 # NOTE most update to date
            # self.start_idx, self.end_idx = 0, 100 # NOTE for debugging / testing
        else:
            # self.start_idx, self.end_idx = 500_000, 750_000
            self.start_idx, self.end_idx = 500_000, 501_000 # NOTE most up-to-date
    ...
    def _process_dataset(self):
        ...
        for doc_id in tqdm(range(self.start_idx, self.end_idx)):
            ...
            title = doc['title'] + " . "
            ingredients = (', '.join(doc['ner']) + " . ").capitalize()
            directions = "".join([d[:-1] + " . " for d in doc['directions']])

            for row, label in zip([title, ingredients, directions], [0, 1, 2]):
                tokenized_text = self.tokenizer.convert_tokens_to_ids(
                    self.tokenizer.tokenize(row))
                text = row
...
class WikihowClassificationDataset(RecipeClassificationDataset):
    ...
    def __init__(self,
        ...
        super(WikihowClassificationDataset, self).__init__(
                 tokenizer=tokenizer,
                 file_path=file_path,
                 block_size=block_size,
                 overwrite_cache=overwrite_cache,
    ...
    def _set_indices(self):
        ...
        if self.train:
            # self.start_idx, self.end_idx = 0, 40_000
            # self.start_idx, self.end_idx = 0, 10_000
            self.start_idx, self.end_idx = 0, 600
        else:
            # self.start_idx, self.end_idx = 40_000, 41_000
            self.start_idx, self.end_idx = 40_000, 40_100
    ...
    def _process_dataset(self):
        ...
        for doc_id in tqdm(range(self.start_idx, self.end_idx)):
            ...
            for method_name, steps in method2steps.items():
                ...
                for text, label in zip([title, method], [0, 1]):
                ...
                # NOTE not using all the steps because too many, imbalance
                step_num = random.randint(0, len(steps)-1)
                step = steps[step_num]
                summary = [ f"{step_num} " + step['summary'][:-1] + " . "]
                sentences = [_ + " . " for _ in step['text'].split(split_pattern)]
                if sentences[-1].endswith(". . "):
                    sentences[-1] = sentences[-1].replace('. . ', ' . ')
                step_text = summary + sentences
                step_text = "".join(step_text)

                if self.is_valid(step_text):
                    tokenized = self.tokenizer(step_text,
                                          padding='max_length',
                                          max_length=self.max_seq_len,
                                          truncation=True)['input_ids']
                    self.examples.append(tokenized)
    ...
    def is_valid(self, text):
        ...
        return len(tokenized_text) < self.block_size
