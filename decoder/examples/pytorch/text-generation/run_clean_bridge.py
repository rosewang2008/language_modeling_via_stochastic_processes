...
python run_clean_bridge.py --model_type=gpt2 --model_name_or_path="/nlp/scr/rewang/transformers/examples/pytorch/language-modeling/cl_wikisection_splitsection" --prompt="<|endoftext|>" --length=100 --dataset_name=wikisection_splitsection  --method=sample
"""
...
def doc2section(dataset, index, dataset_name):
    if "fulldoc" in dataset_name:
        example = dataset.raw_texts[index]
        raw_text_0 = example[:example.find(WIKI_ORDER[1])]
        raw_text_1 = example[
            example.find(WIKI_ORDER[1]):example.find(WIKI_ORDER[2])]
        raw_text_2 = example[
            example.find(WIKI_ORDER[2]):example.find(WIKI_ORDER[3])]
        raw_text_3 = example[
            example.find(WIKI_ORDER[3]):]
        raw_text = [raw_text_0, raw_text_1, raw_text_2, raw_text_3]
    else:
        raw_text = [dataset.examples[index+idx] for idx in range(4)]
    return raw_text
...
def main():
    ...
    parser.add_argument("--block_size", type=int, default=1024, help="random seed for initialization")
    ...
    parser.add_argument("--use_bos", action="store_true", help="Avoid using CUDA when available")
    ...
    if "fulldoc" in args.dataset_name:
        skip_int = 1
    else:
        skip_int = 4


    for example_id in range(0, len(eval_dataset), skip_int):
        ...
        (raw_text_0, raw_text_1, raw_text_2, raw_text_T) = doc2section(
            dataset=eval_dataset, index=example_id, dataset_name=args.dataset_name)
        ...
        section_bridge = simulate_brownian_bridge(
            B_0=section_0_feats[0][0],
            B_T=section_T_feats[0][0],
            num_samples=4)[1:-1] # ignore first and last elements

        bridge += [[_] for _ in section_bridge]
        bridge += section_T_feats
        full_split_raw_text = [raw_text_0_, [split_text(raw_text_1)[0]], [split_text(raw_text_2)[0]], raw_text_T_]
        ...
        for t, cl_feat in enumerate(bridge):
            ...
            for feat_num, feat in enumerate(cl_feat):
                ...
                # Try BOS on first section, no BOS on others
                if t == 0:
                    prompt_text = f"{tokenizer.bos_token}"
                    encoded_prompt = tokenizer.encode(
