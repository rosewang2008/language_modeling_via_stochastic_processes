...
def main():
    ...
    for  _ in tqdm.tqdm(range(num_intervals)):
        # Note: using the first sentence for embedding
        ...
        if not split_text[0]: # empty string
            split_text = split_text[1:]

        cl_input_ids, cl_attention_mask = model.transformer.cl_tokenize_text([split_text[0]])
        cl_feats = REAL_CL_MODEL.forward(input_ids=cl_input_ids, attention_mask=cl_attention_mask) # 1, feat_size
        print(cl_feats)
