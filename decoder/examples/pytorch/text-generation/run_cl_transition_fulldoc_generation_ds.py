...
def load_cl_model(filepath):
    import sys
    sys.path.append('/nlp/scr/rewang/nonstationarity_taskmaster_movies/code/')
...
def main():
    ...
    for  _ in tqdm.tqdm(range(num_intervals)):
        ...
        for seq_i, (seq_cl_feats, tracker) in enumerate(zip(feats, trackers)):
            ...
            print('ranges', min_length, max_length)
...
def main():
    ...
    for  _ in tqdm.tqdm(range(num_intervals)):
        ...
        for seq_i, (seq_cl_feats, tracker) in enumerate(zip(feats, trackers)):
            ...
            model.transformer._cur_cl_idx = 0
            model.transformer._has_reset = False
            print('ranges', min_length, min(length + len(encoded_prompt[0]), 1024))
...
if __name__ == "__main__":
    main()
>>>>>>> origin/replicate_taskmaster_domain_agnostic
||||||| 6fe02bad2
=======
#!/usr/bin/env python
# coding=utf-8
...
def load_cl_model(filepath):
    import sys
    sys.path.append('/nlp/scr/rewang/nonstationarity_recipe/code/')
    from src.models import language
    model = language.GPT2OUEncoder(
         hidden_dim=HIDDEN_DIM,
         latent_dim=LATENT_DIM,
...
if __name__ == "__main__":
    main()
>>>>>>> origin/replicate_recipe_domain_agnostic
