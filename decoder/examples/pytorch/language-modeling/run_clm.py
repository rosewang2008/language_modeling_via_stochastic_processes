...
def main():
    ...
    if data_args.dataset_name == "stories":
        tokenizer.add_tokens(['[ WP ]', '[ RESPONSE ]'])
    # if data_args.dataset_name == "wikisection":
    #     tokenizer.add_tokens(['[ ABSTRACT ]', '[ HISTORY ]', '[ GEOGRAPHY ]', '[ DEMOGRAPHICS ]'])
        SPECIAL_TOKENS = []
    if "wikisection" in data_args.dataset_name:
        tokenizer.add_tokens(SECTION_IDS)

    SPECIAL_TOKENS = [_[0] for _ in tokenizer(SECTION_IDS)['input_ids']]

    ### Expand tokenizer for stories
